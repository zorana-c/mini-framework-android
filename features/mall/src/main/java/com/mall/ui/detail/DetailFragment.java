...
import com.common.ui.adapter.view.CommodityViewHolder;
import com.common.widget.SaturatedRecyclerView;
...
import com.framework.widget.sliver.SliverSmoothHelper;
import com.mall.R;
 ...
 * @Description :
 */
public class DetailFragment extends UIListFragment<Commodity> {
    @Override
    ...
    public void onUICreated(@Nullable Bundle savedInstanceState) {
        ...
        final SliverSmoothHelper helper = new SliverSmoothHelper();
        helper.attachToSliverContainer(this.getUIPageController().getSliverRefreshLayout());
        this.getUIPageController().addOnScrollListener(new SliverContainer.SimpleOnScrollListener() {
            @Override
            public void onScrolled(@NonNull SliverContainer sliverContainer, int dx, int dy) {
                UILog.e("FinalY: " + helper.getFinalY(R.id.test1));
            }
        });
    }

    private void initBanner() {
        final BannerRecyclerView bannerRecyclerView = this.requireViewById(R.id.bannerRecyclerView);
        BannerAdapter<Banner> adapter = (BannerAdapter<Banner>) bannerRecyclerView.getAdapter();
        if (adapter == null) {
            adapter = new BannerAdapter<>();

            bannerRecyclerView.setHasFixedSize(true);
            bannerRecyclerView.setLifecycle(this);
    ...
    private void initInfoRec() {
        final SaturatedRecyclerView saturatedRecyclerView = this.requireViewById(R.id.infoRecyclerView);
        DetailAdapter adapter = (DetailAdapter) saturatedRecyclerView.getAdapter();
        if (adapter == null) {
            adapter = new DetailAdapter();

            saturatedRecyclerView.setHasFixedSize(true);
            saturatedRecyclerView.setAdapter(adapter);
        }
        adapter.getUIListDataController().set("");
    }

    private void initInfoWeb() {
        final WebView webView = this.requireViewById(R.id.infoWebView);
        ...
        // 设置数据来源
        webView.loadData("<p style=\"text-align:center;\">富有传奇色彩的“尚方宝剑”</p><p><br/></p><p " +
